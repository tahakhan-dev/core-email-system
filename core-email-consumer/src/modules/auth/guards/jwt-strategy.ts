...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt') {
    constructor(
        @Inject(CacheEmailService) private readonly userEmailService: CacheEmailService,
        ...
        super({
            ...
            secretOrKeyProvider: async (request, token, done) => {  // By using the secretOrKeyProvider option in the JwtStrategy, you can dynamically provide the secret key based on the user's data or any other logic you want to implement.
                let DecryptToken: IDecryptWrapper, cacheResult, userSecretKey: string;
                ...
                if (DecryptToken) { // If the JWT token exists, perform the specified actions.
                    cacheResult = JSON.parse(await this.userEmailService?.getUserProfileRedis(DecryptToken.id)) // Retrieving the consumerProfile cache result from Redis.
