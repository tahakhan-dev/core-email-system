...
import { CacheUserService } from "../../../common/cache/cache-user.service";
...
export class JwtAuthGuard extends AuthGuard('jwt') { // AuthGuard class is a built-in class provided by NestJS that acts as a base class for creating custom authentication guards.
    constructor(
        ...
        @Inject(CacheUserService) private readonly cacheUserService: CacheUserService // An instance of the CacheUserService class, which provides caching functionality related to user data
    ) {
    ...
    async canActivate(context: ExecutionContext): Promise<any> { // it is responsible for determining whether a user is authorized to access a specific route or resource
        ...
        const checkingValidation: boolean = await this.cacheUserService?.checkUniqueIdentifierAndTokenIssueFromSameUser(userUniqueIdentifier, tokenDetail?.id) // Checking if the token is generated from the same device. If it is generated from a different device, throw an error indicating that the user needs to register their device by signing in with that device first
